...
export function estimateCostUsd(
  ...
  if (
    configCost &&
    (configCost.input !== undefined || configCost.output !== undefined)
  ) {
...
function estimateCostRates(model: string): [number, number] {
  ...
  if (model.includes("opus-4-6") || model.includes("claude-opus-4-6"))
    return [5.0, 25.0];
  ...
  if (model.includes("gpt-4o")) return [2.5, 10.0];
  if (model.includes("gpt-4.1-nano")) return [0.1, 0.4];
  if (model.includes("gpt-4.1-mini")) return [0.4, 1.6];
  if (model.includes("gpt-4.1")) return [2.0, 8.0];
  if (model.includes("gpt-5-nano")) return [0.05, 0.4];
  if (model.includes("gpt-5-mini")) return [0.25, 2.0];
  if (model.includes("gpt-5")) return [1.25, 10.0];
  if (model.includes("o3-mini") || model.includes("o4-mini")) return [1.1, 4.4];
  if (model.includes("gemini-2.5-flash-lite")) return [0.04, 0.15];
  if (model.includes("gemini-2.5-pro")) return [1.25, 10.0];
  if (model.includes("gemini-2.5-flash")) return [0.15, 0.6];
  if (model.includes("gemini-2.0-flash") || model.includes("gemini-flash"))
    return [0.1, 0.4];
  if (model.includes("gemini")) return [0.15, 0.6];
  if (model.includes("deepseek-reasoner") || model.includes("deepseek-r1"))
    return [0.55, 2.19];
  if (model.includes("deepseek")) return [0.27, 1.1];
  if (model.includes("llama-4-maverick")) return [0.5, 0.77];
  if (model.includes("llama-4-scout")) return [0.11, 0.34];
  if (model.includes("llama") || model.includes("mixtral")) return [0.05, 0.1];
  if (model.includes("qwen-max")) return [4.0, 12.0];
  if (model.includes("qwen-plus")) return [0.8, 2.0];
  if (model.includes("qwen")) return [0.2, 0.6];
  if (model.includes("glm-4-flash") || model.includes("glm-4.7-flash"))
    return [0.1, 0.1];
  if (model.includes("glm")) return [1.5, 5.0];
  if (model.includes("mistral-large")) return [2.0, 6.0];
  if (model.includes("mistral")) return [0.1, 0.3];
  if (model.includes("grok-4-1")) return [0.2, 0.5];
  if (model.includes("grok-4") || model.includes("grok-3")) return [3.0, 15.0];
  if (model.includes("grok")) return [2.0, 10.0];
  if (model.includes("command-r-plus")) return [2.5, 10.0];
  if (model.includes("command-r")) return [0.15, 0.6];
